...
/**
 * 第一重缝合：存储下游上下文（明文 Base64URL，通过企业微信 state 传递）
 */
...
// ============================================================================
// 企业微信 API 调用（client_secret 仅在内存中，请求结束即释放）
// ============================================================================
...
async function fetchAccessToken(
  ...
  const url = new URL(WECOM_ACCESS_TOKEN_URL);
  url.searchParams.set('corpid', corpId);
  url.searchParams.set('corpsecret', corpSecret);

  const resp = await fetch(url.toString(), { method: 'GET' });
  if (!resp.ok) {
    throw new Error(`wecom gettoken HTTP ${resp.status}`);
  }
  const data = (await resp.json()) as WecomAccessToken;
  if (data.errcode && data.errcode !== 0) {
    throw new Error(`wecom gettoken errcode=${data.errcode} errmsg=${data.errmsg}`);
  }
  if (!data.access_token) {
    throw new Error('wecom gettoken missing access_token');
  }
...
async function fetchUserInfo(
  ...
  const resp = await fetch(url.toString(), { method: 'GET' });
  if (!resp.ok) {
    throw new Error(`wecom user/get HTTP ${resp.status}`);
  }
  const data = (await resp.json()) as WecomUserInfo;
  if (data.errcode && data.errcode !== 0) {
    throw new Error(`wecom user/get errcode=${data.errcode} errmsg=${data.errmsg}`);
  }
  return data;
}
...
export const wecomProvider: Provider = {
  ...
  /**
   * GET /wecom/api/callback
   *
   * 第二重缝合：解缝合企业微信返回的 state，签发 packed_code JWT，重定向回下游。
   */
  async handleCallback(c: Context<AppEnv>): Promise<Response> {
    const code = c.req.query('code');
    const wecomState = c.req.query('state');
  ...
  /**
   * POST /wecom/api/token
   *
   * 凭证透传：验证 packed_code 签名，校验 redirect_uri/client_id，
   * 用下游传来的 client_secret 调用企业微信 API，签发 id_token。
   */
  async exchangeCode(c: Context<AppEnv>): Promise<Response> {
    ...
    try {
      ...
      return c.json(
        {
          error: 'invalid_grant',
          error_description: e instanceof Error ? e.message : 'wecom API error',
        },
        502,
      );
    }
